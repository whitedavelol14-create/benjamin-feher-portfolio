...
export class SqliteLevel extends AbstractLevel {
    ...
    async _open(options, callback) {
        ...
        const hasUniqueOnKey = this.db
            .prepare(`SELECT 1 FROM pragma_index_list('kv') il
          WHERE il."unique" = 1
            AND il.partial = 0
        ...
        if (!hasUniqueOnKey) {
            const migrate = this.db.transaction(() => {
                this.db.exec(`DELETE FROM kv WHERE rowid NOT IN (SELECT MAX(rowid) FROM kv GROUP BY key);
