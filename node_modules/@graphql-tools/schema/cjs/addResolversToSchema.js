...
const LEGACY_TO_V17_SCALAR_METHOD = {
    serialize: 'coerceOutputValue',
    parseValue: 'coerceInputValue',
};
function setScalarConfigProperty(target, propName, value) {
    ...
    schema = (0, utils_1.mapSchema)(schema, {
        [utils_1.MapperKind.SCALAR_TYPE]: type => {
            ...
            if (!(0, graphql_1.isSpecifiedScalarType)(type) && resolverValue != null) {
                const coerceInputValueOverridden = ['parseValue', 'coerceInputValue'].some(name => name in resolverValue || `__${name}` in resolverValue);
                const parseLiteralOverridden = ['parseLiteral', 'coerceInputLiteral'].some(name => name in resolverValue || `__${name}` in resolverValue);
                for (const fieldName in resolverValue) {
                    if (fieldName.startsWith('__')) {
