...
import { isDefined as isDefined6, NodeApi as NodeApi3 } from "@udecode/plate";
...
var getIndentListAbove = (editor, options) => {
  return editor.api.above({
    ...options,
    match: (node) => isDefined5(node[BaseIndentListPlugin.key])
  });
};
  ...
  return {
    transforms: {
      deleteBackward(unit) {
        ...
        if (isDefined6(listNode[BaseIndentListPlugin.key])) {
...
import { isDefined as isDefined7 } from "@udecode/plate";
  ...
  return {
    transforms: {
      insertBreak() {
        ...
        if (!isDefined7(node[BaseIndentListPlugin.key]) || node[BaseIndentListPlugin.key] !== INDENT_LIST_KEYS.todo || editor.api.isExpanded() || !editor.api.isEnd(editor.selection?.focus, path)) {
          return insertBreak();
        }
        editor.tf.withoutNormalizing(() => {
          insertBreak();
