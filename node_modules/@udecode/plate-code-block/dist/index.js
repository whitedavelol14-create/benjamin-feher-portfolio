...
var isCodeBlockEmpty = (editor) => {
  ...
  const codeLines = Array.from(import_plate3.NodeApi.children(editor, codeBlock[1]));
  if (codeLines.length === 0) return true;
  if (codeLines.length > 1) return false;
  const firstCodeLineNode = codeLines[0][0];
  return !import_plate3.NodeApi.string(firstCodeLineNode);
};
...
var insertEmptyCodeBlock = (editor, {
  defaultType = editor.getType(import_plate4.BaseParagraphPlugin),
  insertNodesOptions
} = {}) => {
...
var unwrapCodeBlock = (editor) => {
  ...
  const defaultType = editor.getType(import_plate5.BaseParagraphPlugin);
  editor.tf.withoutNormalizing(() => {
    ...
    for (const codeBlockEntry of reversedCodeBlockEntries) {
      const codeLineEntries = import_plate5.NodeApi.children(editor, codeBlockEntry[1]);
  ...
  transforms: {
    insertFragment(fragment) {
      ...
      function convertNodeToCodeLine(node) {
        return {
          children: [{ text: import_plate6.NodeApi.string(node) }],
          type: codeLineType
        };
      }
...
var withNormalizeCodeBlock = ({
  ...
  transforms: {
    normalizeNode([node, path]) {
      normalizeNode([node, path]);
      if (!import_plate7.ElementApi.isElement(node)) {
        return;
      }
      const codeBlockType = editor.getType(BaseCodeBlockPlugin);
      const codeLineType = editor.getType(BaseCodeLinePlugin);
      ...
      if (isCodeBlockRoot) {
        const nonCodeLine = Array.from(import_plate7.NodeApi.children(editor, path)).find(
...
// src/lib/BaseCodeBlockPlugin.ts
var BaseCodeLinePlugin = (0, import_plate8.createTSlatePlugin)({
  key: "code_line",
  node: { isElement: true }
});
var BaseCodeSyntaxPlugin = (0, import_plate8.createSlatePlugin)({
  key: "code_syntax",
  node: { isLeaf: true }
});
var BaseCodeBlockPlugin = (0, import_plate8.createTSlatePlugin)({
  key: "code_block",
  inject: {
    plugins: {
      [import_plate8.HtmlPlugin.key]: {
        parser: {
