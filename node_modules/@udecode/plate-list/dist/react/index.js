...
var removeListItem = (editor, { list, listItem, reverse = true }) => {
  ...
  const previousLiPath = import_plate16.PathApi.previous(liPath);
  let success = false;
  editor.tf.withoutNormalizing(() => {
    if (previousLiPath) {
      const previousLi = editor.api.node(previousLiPath);
      if (!previousLi) return;
      let tempLiPath = import_plate16.PathApi.next(liPath);
      editor.tf.insertNodes(
        {
          children: [
            {
      ...
      (0, import_plate16.deleteMerge)(editor, {
        reverse
      });
...
var toggleList = (editor, { type }) => editor.tf.withoutNormalizing(() => {
  ...
  if (editor.api.isCollapsed() || !editor.api.isAt({ blocks: true })) {
    ...
    if (res) {
      ...
      if (list[0].type === type) {
        ...
        editor.tf.setNodes(
          { type },
          {
            at: editor.selection,
            mode: "lowest",
            match: (n) => import_plate17.ElementApi.isElement(n) && getListTypes(editor).includes(n.type)
          }
        );
      }
    } else {
      ...
      const _nodes = editor.api.nodes({
        match: { type: editor.getType(import_plate17.BaseParagraphPlugin) }
      });
    ...
    const [startPoint, endPoint] = import_plate17.RangeApi.edges(editor.selection);
    const commonEntry = import_plate17.NodeApi.common(
      editor,
      startPoint.path,
      endPoint.path
    );
    if (getListTypes(editor).includes(commonEntry[0].type) || commonEntry[0].type === editor.getType(BaseListItemPlugin)) {
      if (commonEntry[0].type === type) {
        ...
        const startList = editor.api.node({
          at: import_plate17.RangeApi.start(editor.selection),
          match: { type: getListTypes(editor) },
          mode: "lowest"
        });
        const endList = editor.api.node({
          at: import_plate17.RangeApi.end(editor.selection),
          match: { type: getListTypes(editor) },
          mode: "lowest"
        });
        ...
        editor.tf.setNodes(
          { type },
          {
            at: editor.selection,
            mode: "all",
            match: (n, path) => import_plate17.ElementApi.isElement(n) && getListTypes(editor).includes(n.type) && path.length >= rangeLength
          }
        );
      }
    } else {
        ...
        if (getListTypes(editor).includes(n[0].type)) {
          editor.tf.setNodes(
            { type },
            {
              at: n[1],
              mode: "all",
              match: (_n) => import_plate17.ElementApi.isElement(_n) && getListTypes(editor).includes(_n.type)
            }
          );
        } else {
...
// src/lib/BaseListPlugin.ts
var BaseBulletedListPlugin = (0, import_plate18.createSlatePlugin)({
  key: "ul",
  node: { isElement: true },
  parsers: {
    html: {
...
var BaseNumberedListPlugin = (0, import_plate18.createSlatePlugin)({
  key: "ol",
  node: { isElement: true },
  parsers: { html: { deserializer: { rules: [{ validNodeName: "OL" }] } } }
});
var BaseListItemPlugin = (0, import_plate18.createSlatePlugin)({
  key: "li",
  inject: {
    plugins: {
      [import_plate18.HtmlPlugin.key]: {
        parser: {
...
var BaseListItemContentPlugin = (0, import_plate18.createSlatePlugin)({
  key: "lic",
  node: { isElement: true }
});
var BaseListPlugin = (0, import_plate18.createTSlatePlugin)({
  key: "list",
  // TODO react
  ...
  toggle: {
    bulletedList: (0, import_plate18.bindFirst)(toggleBulletedList, editor),
    list: (0, import_plate18.bindFirst)(toggleList, editor),
    numberedList: (0, import_plate18.bindFirst)(toggleNumberedList, editor)
  }
}));
...
var selectionIsInAListHandler = (editor, res, defaultDelete, unit = "character") => {
  ...
  if (!hasListChild(editor, listItem[0])) {
    ...
    const _nodes = editor.api.nodes({
      ...
      match: (node, path) => {
        ...
        const isSiblingOfNodeLi = import_plate19.NodeApi.get(editor, import_plate19.PathApi.next(path))?.type === liType;
    ...
    const siblingListItem = editor.api.node(
      import_plate19.PathApi.next(liWithSiblings)
    );
    ...
    const leftoverListItem = editor.api.node(
      import_plate19.PathApi.parent(nextSelectableLic[1])
    );
  ...
  const nestedList = editor.api.node(
    import_plate19.PathApi.next([...listItem[1], 0])
  );
  if (!nestedList) return false;
  const nestedListItem = Array.from(
    import_plate19.NodeApi.children(editor, nestedList[1])
  )[0];
    ...
    deleteFragment(direction) {
      const deleteFragmentList = () => {
        ...
        editor.tf.withoutNormalizing(() => {
          ...
          (0, import_plate20.deleteMerge)(editor);
...
var withInsertFragmentList = ({
  ...
  const getFirstAncestorOfType = (root, entry, type) => {
    let ancestor = import_plate21.PathApi.parent(entry[1]);
    while (import_plate21.NodeApi.get(root, ancestor).type !== type) {
      ancestor = import_plate21.PathApi.parent(ancestor);
    }
    return [import_plate21.NodeApi.get(root, ancestor), ancestor];
  };
  ...
  const trimList = (listRoot) => {
    ...
    const _texts = import_plate21.NodeApi.texts(listRoot);
    const textEntries = Array.from(_texts);
    const commonAncestorEntry = textEntries.reduce(
      (commonAncestor, textEntry) => import_plate21.PathApi.isAncestor(commonAncestor[1], textEntry[1]) ? commonAncestor : import_plate21.NodeApi.common(listRoot, textEntry[1], commonAncestor[1]),
  ...
  const isSingleLic = (fragment) => {
    ...
    return isFragmentOnlyListRoot && [...import_plate21.NodeApi.nodes({ children: fragment })].filter(
      (entry) => import_plate21.ElementApi.isElement(entry[0])
    ).filter(([node]) => node.type === listItemContentType).length === 1;
  };
  const getTextAndListItemNodes = (fragment, liEntry, licEntry) => {
    const [, liPath] = liEntry;
    const [licNode, licPath] = licEntry;
    const isEmptyNode = !import_plate21.NodeApi.string(licNode);
    ...
    transforms: {
      insertFragment(fragment) {
        ...
        return editor.tf.insertNodes(listItemNodes, {
          at: import_plate21.PathApi.next(liPath),
          select: true
        });
      }
    }
    ...
    if (editor.api.isBlock(child[0])) {
      inlineChildren.push(
        ...getDeepInlineChildren(editor, {
          children: Array.from(import_plate22.NodeApi.children(editor, child[1]))
        })
      );
    } else {
...
var normalizeListItem = (editor, {
  ...
  const liChildren = Array.from(
    import_plate22.NodeApi.children(editor, listItem[1])
  );
  ...
  if (editor.api.isBlock(firstLiChildNode) && !(0, import_plate22.match)(firstLiChildNode, [], {
    type: editor.getType(BaseListItemContentPlugin)
  })) {
    if ((0, import_plate22.match)(firstLiChildNode, [], {
      type: getListTypes(editor)
    })) {
      const parent = editor.api.parent(listItem[1]);
      const sublist = firstLiChild;
      const children = Array.from(
        import_plate22.NodeApi.children(editor, firstLiChild[1])
      ).reverse();
      children.forEach((c) => {
  ...
  const licChildren = Array.from(import_plate22.NodeApi.children(editor, firstLiChild[1]));
  if (licChildren.length > 0) {
    ...
    for (const licChild of licChildren) {
      ...
      inlineChildren.push(
        ...getDeepInlineChildren(editor, {
          children: Array.from(import_plate22.NodeApi.children(editor, licChild[1]))
        })
      );
    }
    const to = import_plate22.PathApi.next(licChildren.at(-1)[1]);
...
var normalizeNestedList = (editor, { nestedListItem }) => {
  ...
  const hasParentList = parentNode && (0, import_plate23.match)(parentNode[0], [], { type: getListTypes(editor) });
  if (!hasParentList) {
    return false;
  }
  const previousListItemPath = import_plate23.PathApi.previous(path);
  if (!previousListItemPath) {
    return false;
  }
  const previousSiblingItem = editor.api.node(previousListItemPath);
...
var withNormalizeList = ({
  ...
  transforms: {
    normalizeNode([node, path]) {
      const liType = editor.getType(BaseListItemPlugin);
      const licType = editor.getType(BaseListItemContentPlugin);
      const defaultType = editor.getType(import_plate24.BaseParagraphPlugin);
      if (!import_plate24.ElementApi.isElement(node)) {
        return normalizeNode([node, path]);
      }
      if (isListRoot(editor, node)) {
        const nonLiChild = Array.from(import_plate24.NodeApi.children(editor, path)).find(
      ...
      if ((0, import_plate24.match)(node, [], { type: getListTypes(editor) })) {
        if (node.children.length === 0 || !node.children.some((item) => item.type === liType)) {
          return editor.tf.removeNodes({ at: path });
        }
        const nextPath = import_plate24.PathApi.next(path);
        const nextNode = import_plate24.NodeApi.get(editor, nextPath);
        ...
        const prevPath = import_plate24.PathApi.previous(path);
        const prevNode = import_plate24.NodeApi.get(editor, prevPath);
...
var onKeyDownList = ({
  ...
  const isTab = import_plate25.Hotkeys.isTab(editor, event);
  const isUntab = import_plate25.Hotkeys.isUntab(editor, event);
  let workRange = editor.selection;
  if (editor.selection && (isTab || isUntab)) {
    const { selection } = editor;
    if (!editor.api.isCollapsed()) {
      const { anchor, focus } = import_plate25.RangeApi.isBackward(selection) ? { anchor: { ...selection.focus }, focus: { ...selection.anchor } } : { anchor: { ...selection.anchor }, focus: { ...selection.focus } };
...
var import_react = require("@udecode/plate-reset-node/react");
        ...
        if (res) {
          ...
          if (editor.api.isAt({
            ...
            editor.tf.withoutNormalizing(() => {
              ...
              if (moved) return true;
              if (!import_plate26.PathApi.hasPrevious(listItem[1]) && !isListNested(editor, list[1])) {
                (0, import_react.onKeyDownResetNode)({
                  ...(0, import_react2.getEditorPlugin)(
                    editor,
                    import_plate_reset_node.BaseResetNodePlugin.configure({
                      options: {
                        rules: [
                          {
                            defaultType: editor.getType(import_plate26.BaseParagraphPlugin),
                            hotkey: "backspace",
                  ...
                  event: import_react.SIMULATE_BACKSPACE
                });
                moved = true;
                return;
              }
              ...
              (0, import_plate26.deleteMerge)(editor, {
                reverse: true,
                unit
              });
              moved = true;
              ...
              const leftoverListItem = editor.api.node(
                import_plate26.PathApi.parent(currentLic[1])
              );
              if (leftoverListItem && leftoverListItem[0].children.length === 0) {
                editor.tf.removeNodes({ at: leftoverListItem[1] });
              }
...
var import_react3 = require("@udecode/plate-reset-node/react");
  ...
  transforms: {
    insertBreak() {
      const insertBreakList = () => {
        ...
        const didReset = (0, import_react3.onKeyDownResetNode)({
          ...(0, import_react4.getEditorPlugin)(
            editor,
            (0, import_plate27.createTSlatePlugin)({
              options: {
                rules: [
                  {
                    defaultType: editor.getType(import_plate27.BaseParagraphPlugin),
                    types: [editor.getType(BaseListItemPlugin)],
          ...
          event: import_react3.SIMULATE_BACKSPACE
        });
...
// src/react/ListPlugin.tsx
var BulletedListPlugin = (0, import_react5.toPlatePlugin)(BaseBulletedListPlugin, {
  dependencies: ["list"],
  handlers: {
    onKeyDown: onKeyDownList
  }
});
var NumberedListPlugin = (0, import_react5.toPlatePlugin)(BaseNumberedListPlugin, {
  dependencies: ["list"],
  handlers: {
    onKeyDown: onKeyDownList
  }
});
var ListItemContentPlugin = (0, import_react5.toPlatePlugin)(BaseListItemContentPlugin);
var ListItemPlugin = (0, import_react5.toPlatePlugin)(BaseListItemPlugin);
var ListPlugin = (0, import_react5.toPlatePlugin)(BaseListPlugin, {
  plugins: [
    BulletedListPlugin,
    NumberedListPlugin,
    ListItemPlugin,
  ...
  shortcuts: {
    toggleBulletedList: {
      keys: [[import_react5.Key.Mod, import_react5.Key.Alt, "5"]],
      preventDefault: true,
      handler: () => {
    ...
    toggleNumberedList: {
      keys: [[import_react5.Key.Mod, import_react5.Key.Alt, "6"]],
      preventDefault: true,
      handler: () => {
...
var TodoListPlugin = (0, import_react6.toTPlatePlugin)(
  BaseTodoListPlugin,
  ({ editor, type }) => ({
    shortcuts: {
      toggleTodoList: {
        keys: [
          [import_react6.Key.Mod, import_react6.Key.Alt, "4"],
          [import_react6.Key.Mod, import_react6.Key.Shift, "4"]
        ],
        preventDefault: true,
        handler: () => {
...
var useTodoListElementState = ({
  ...
  const readOnly = (0, import_react8.useReadOnly)();
  return {
    checked,
    editor,
    element,
