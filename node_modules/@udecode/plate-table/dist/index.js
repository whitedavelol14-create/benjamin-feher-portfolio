...
var getCellInNextTableRow = (editor, currentRowPath) => {
  const nextRow = editor.api.node(import_plate.PathApi.next(currentRowPath));
  if (!nextRow) return;
...
var getCellInPreviousTableRow = (editor, currentRowPath) => {
  const prevPath = import_plate2.PathApi.previous(currentRowPath);
  if (!prevPath) return;
...
function computeCellIndices(editor, {
  ...
  const { api, getOptions, setOption } = (0, import_plate3.getEditorPlugin)(editor, {
    key: "table"
  });
  if (!tableNode) {
    if (!cellNode) return;
...
var getCellIndices = (editor, element) => {
  const { getOption } = (0, import_plate4.getEditorPlugin)(editor, {
    key: "table"
  });
...
var getCellTypes = (editor) => (0, import_plate5.getPluginTypes)(editor, [BaseTableCellPlugin, BaseTableCellHeaderPlugin]);
...
var getLeftTableCell = (editor, {
  ...
  if (!cellIndex) return;
  const prevCellPath = import_plate6.PathApi.previous(cellPath);
...
var getNextTableCell = (editor, currentCell, currentPath, currentRow) => {
  const cell = editor.api.node(import_plate7.PathApi.next(currentPath));
  if (cell) return cell;
...
var getPreviousTableCell = (editor, currentCell, currentPath, currentRow) => {
  const prevPath = import_plate8.PathApi.previous(currentPath);
  if (!prevPath) {
    const [, currentRowPath] = currentRow;
...
var getTopTableCell = (editor, {
  ...
  const cellAbovePath = [
    ...import_plate9.PathApi.parent(import_plate9.PathApi.parent(cellPath)),
    rowIndex - 1,
    cellIndex
  ];
...
var getTableCellSize = (editor, {
  ...
  const { api } = (0, import_plate10.getEditorPlugin)(editor, {
    key: "table"
  });
...
var getTableMergeGridByRange = (editor, { at, format }) => {
  const { api, type } = (0, import_plate11.getEditorPlugin)(editor, BaseTablePlugin);
...
var getTableGridByRange = (editor, { at, format = "table" }) => {
  ...
  while (true) {
    ...
    const cell = import_plate12.NodeApi.get(editor, cellPath);
    if (!cell) break;
    const rows = table.children[rowIndex - startRowIndex].children;
    rows[colIndex - startColIndex] = cell;
    cellEntries.push([cell, cellPath]);
...
var getTableGridAbove = (editor, { format = "table", ...options } = {}) => {
  ...
  if (edges) {
    ...
    if (!import_plate13.PathApi.equals(start[1], end[1])) {
...
var getTableRowIndex = (editor, cellNode) => {
  const path = editor.api.findPath(cellNode);
  if (!path) return 0;
  const rowPath = import_plate14.PathApi.parent(path);
  return rowPath.at(-1);
};
...
var deleteColumnWhenExpanded = (editor, tableEntry) => {
  const [start, end] = import_plate15.RangeApi.edges(editor.selection);
  const firstRow = import_plate15.NodeApi.child(tableEntry[0], 0);
  const lastRow = import_plate15.NodeApi.child(
    tableEntry[0],
...
var deleteTableMergeColumn = (editor) => {
  ...
  editor.tf.withoutNormalizing(() => {
    const { api } = (0, import_plate16.getEditorPlugin)(editor, BaseTablePlugin);
...
var deleteRowWhenExpanded = (editor, [table, tablePath]) => {
  const { api } = (0, import_plate17.getEditorPlugin)(editor, BaseTablePlugin);
...
var deleteTableMergeRow = (editor) => {
  const { api, tf, type } = (0, import_plate18.getEditorPlugin)(editor, {
    key: "table"
  });
  if (editor.api.some({
    match: { type }
...
var insertTableMergeColumn = (editor, {
  ...
  const { api, getOptions, type } = (0, import_plate19.getEditorPlugin)(editor, BaseTablePlugin);
  const { initialTableWidth, minColumnWidth } = getOptions();
  if (at && !fromCell) {
    const table = import_plate19.NodeApi.get(editor, at);
    if (table?.type === editor.getType(BaseTablePlugin)) {
      fromCell = import_plate19.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  ...
  if (import_plate19.PathApi.isPath(at)) {
...
var insertTableMergeRow = (editor, {
  ...
  const { api, type } = (0, import_plate20.getEditorPlugin)(editor, BaseTablePlugin);
  if (at && !fromRow) {
    const table = import_plate20.NodeApi.get(editor, at);
    if (table?.type === editor.getType(BaseTablePlugin)) {
      fromRow = import_plate20.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  ...
  let nextRowPath;
  if (import_plate20.PathApi.isPath(at)) {
...
var mergeTableCells = (editor) => {
  const { api } = (0, import_plate21.getEditorPlugin)(editor, BaseTablePlugin);
...
var splitTableCell = (editor) => {
  const { api } = (0, import_plate22.getEditorPlugin)(editor, BaseTablePlugin);
...
var deleteColumn = (editor) => {
  const { getOptions, type } = (0, import_plate23.getEditorPlugin)(editor, {
    key: "table"
  });
...
var deleteRow = (editor) => {
  const { getOptions, type } = (0, import_plate24.getEditorPlugin)(editor, {
    key: "table"
  });
  const { disableMerge } = getOptions();
  if (!disableMerge) {
  ...
  editor.tf.withoutNormalizing(() => {
    ...
    if (!options.at) {
      ...
      if (currentTableEntry) {
        ...
        const insertPath = import_plate25.PathApi.next(tablePath);
...
var insertTableColumn = (editor, options = {}) => {
  const { api, getOptions, type } = (0, import_plate26.getEditorPlugin)(editor, BaseTablePlugin);
  ...
  if (at && !fromCell) {
    const table = import_plate26.NodeApi.get(editor, at);
    if (table?.type === editor.getType(BaseTablePlugin)) {
      fromCell = import_plate26.NodeApi.lastChild(editor, at.concat([0]))[1];
      at = void 0;
    }
  }
  ...
  let nextCellPath;
  let nextColIndex;
  if (import_plate26.PathApi.isPath(at)) {
    nextCellPath = at;
    nextColIndex = at.at(-1);
  } else {
    nextCellPath = before ? cellPath : import_plate26.PathApi.next(cellPath);
...
var insertTableRow = (editor, options = {}) => {
  ...
  if (at && !fromRow) {
    ...
    if (table?.type === editor.getType(BaseTablePlugin)) {
      fromRow = import_plate27.NodeApi.lastChild(editor, at)[1];
      at = void 0;
    }
  }
  const trEntry = editor.api.block({
  ...
  editor.tf.withoutNormalizing(() => {
    editor.tf.insertNodes(getEmptyRowNode2(), {
      at: import_plate27.PathApi.isPath(at) ? at : before ? trPath : import_plate27.PathApi.next(trPath)
    });
  });
  if (shouldSelect) {
    const cellEntry = editor.api.block({
...
var moveSelectionFromCell = (editor, {
  ...
  if (edge) {
    ...
    if (cellEntries.length > minCell) {
      ...
      if (import_plate28.NodeApi.has(editor, anchorPath) && import_plate28.NodeApi.has(editor, focusPath)) {
        editor.tf.select({
  ...
  if (cellEntry) {
    ...
    if (import_plate28.NodeApi.has(editor, nextCellPath)) {
...
var overrideSelectionFromCell = (editor, newSelection) => {
  ...
  if (!editor.currentKeyboardEvent || !["up", "down", "shift+up", "shift+right", "shift+down", "shift+left"].some(
    (key) => {
      const valid = (0, import_plate29.isHotkey)(key, editor.currentKeyboardEvent);
      if (valid) hotkey = key;
      return valid;
    }
...
var setBorderSize = (editor, size, {
  ...
  const setNodesOptions = {
    match: (n) => import_plate30.ElementApi.isElement(n) && getCellTypes(editor).includes(n.type)
  };
  if (border === "top") {
  ...
  transforms: {
    apply(op) {
      if (op.type === "set_selection" && op.newProperties) {
        ...
        if (import_plate31.RangeApi.isRange(newSelection) && editor.api.isAt({
          at: newSelection,
          blocks: true,
          ...
          if (anchorEntry) {
            ...
            const isBackward = import_plate31.RangeApi.isBackward(newSelection);
            if (isBackward) {
...
var preventDeleteTableCell = (editor, {
  ...
  if (editor.api.isCollapsed()) {
    ...
    if (cellEntry) {
      ...
      if (selection && import_plate32.PointApi.equals(selection.anchor, start)) {
        return true;
      }
    } else {
            ...
            editor.tf.withoutNormalizing(() => {
              ...
              insertedRows.forEach((row) => {
                ...
                if (!initRow) {
                  ...
                  if (!import_plate33.NodeApi.has(editor, cellPath)) {
    ...
    marks() {
      ...
      matchesCell.forEach(([_cell, cellPath]) => {
        const textNodeEntry = editor.api.nodes({
          at: cellPath,
          match: (n) => import_plate34.TextApi.isText(n)
        });
...
var withNormalizeTable = ({
  ...
  transforms: {
    normalizeNode([n, path]) {
      ...
      if (import_plate35.ElementApi.isElement(n)) {
        if (n.type === type) {
          const node = n;
          if (!node.children.some(
            (child) => import_plate35.ElementApi.isElement(child) && child.type === editor.getType(BaseTableRowPlugin)
          )) {
            editor.tf.removeNodes({ at: path });
            return;
          }
        ...
        if (getCellTypes(editor).includes(n.type)) {
          ...
          if (import_plate35.TextApi.isText(children[0])) {
...
var BaseTableRowPlugin = (0, import_plate36.createSlatePlugin)({
  key: "tr",
  node: { isElement: true },
  parsers: {
    html: {
...
var BaseTableCellPlugin = (0, import_plate36.createSlatePlugin)({
  key: "td",
  node: {
...
var BaseTableCellHeaderPlugin = (0, import_plate36.createSlatePlugin)({
  key: "th",
  node: {
    dangerouslyAllowAttributes: ["colspan", "rowspan"],
    isElement: true,
...
var BaseTablePlugin = (0, import_plate36.createTSlatePlugin)({
  key: "table",
  ...
  create: {
    table: (0, import_plate36.bindFirst)(getEmptyTableNode, editor),
    tableCell: (0, import_plate36.bindFirst)(getEmptyCellNode, editor),
    tableRow: (0, import_plate36.bindFirst)(getEmptyRowNode, editor)
  },
  table: {
    getCellBorders: (0, import_plate36.bindFirst)(getTableCellBorders, editor),
    getCellSize: (0, import_plate36.bindFirst)(getTableCellSize, editor),
    getColSpan,
    getRowSpan,
  ...
  insert: {
    table: (0, import_plate36.bindFirst)(insertTable, editor),
    tableColumn: (0, import_plate36.bindFirst)(insertTableColumn, editor),
    tableRow: (0, import_plate36.bindFirst)(insertTableRow, editor)
  },
  remove: {
    table: (0, import_plate36.bindFirst)(deleteTable, editor),
    tableColumn: (0, import_plate36.bindFirst)(deleteColumn, editor),
    tableRow: (0, import_plate36.bindFirst)(deleteRow, editor)
  },
  table: {
    merge: (0, import_plate36.bindFirst)(mergeTableCells, editor),
    split: (0, import_plate36.bindFirst)(splitTableCell, editor)
  }
