...
class TypeScriptOperationVariablesToObject extends visitor_plugin_common_1.OperationVariablesToObject {
    ...
    getScalar(name) {
        return this._scalars[name]?.input ?? visitor_plugin_common_1.DEFAULT_INPUT_SCALARS[name].input ?? 'unknown';
    }
    getPunctuation() {
        return ';';
    }
    ...
    wrapMaybe(type) {
        return (0, visitor_plugin_common_1.printTypeScriptMaybeType)({
            type,
