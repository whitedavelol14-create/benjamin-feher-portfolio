...
class SelectionSetToObject {
    ...
    _collectInlineFragments(parentType, nodes, types) {
         ...
         */
        const enrichFieldWithDirectives = ({ node, fields, }) => {
                    ...
                    if (conditionalDirectivesFound) {
                        ...
                        for (const conditionalNode of selectionNodes) {
                            ...
                            if (this._config.inlineFragmentTypes === 'mask' &&
                                ...
                                const { fields: partialFields, dependentTypes: partialDependentTypes } = this.buildSelectionSet(schemaType, [conditionalNode], {
                                    partialTypes: true,
    ...
    buildSelectionSet(parentSchemaType, selectionNodes, options) {
        ...
        for (const selectionNode of selectionNodes) {
            ...
            if (parentSchemaType.name === selectionNode.onType ||
                ...
                const { selectionNodesByTypeName, selectionNodesByTypeNameConditional } = this.flattenSelectionSet(selectionNode.selectionNodes, parentSchemaType);
                ...
                for (const conditionalNodes of selectionNodesByTypeNameConditional) {
                    const _selectionNodes = (conditionalNodes.get(parentSchemaType.name) || []).filter((node) => 'fragmentDirectives' in node);
        ...
        const transformedTypenameFields = typeInfoField &&
            ...
            : [];
        const transformedPrimitiveFields = this._processor.transformPrimitiveFields(parentSchemaType, Array.from(primitiveFields.values()).map(field => ({
            isConditional: (0, utils_js_1.hasConditionalDirectives)(field.directives) ||
                (0, utils_js_1.hasConditionalDirectives)(field.fragmentDirectives),
            fieldName: field.name.value,
        })), options.unsetTypes);
        const transformedAliasesPrimitiveFields = this._processor.transformAliasesPrimitiveFields(parentSchemaType, Array.from(primitiveAliasFields.values()).map(field => ({
            alias: field.alias.value,
            fieldName: field.name.value,
            isConditional: (0, utils_js_1.hasConditionalDirectives)(field.directives) ||
                (0, utils_js_1.hasConditionalDirectives)(field.fragmentDirectives),
        })), options.unsetTypes);
        const transformedLinkFields = this._processor.transformLinkFields(linkFields, options.unsetTypes);
        const transformed = [
