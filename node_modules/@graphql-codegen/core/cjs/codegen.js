...
async function codegen(options) {
    ...
    for (const plugin of pluginPackages) {
        const addToSchema = typeof plugin.addToSchema === 'function'
    ...
    const schemaDocumentNode = mergeNeeded || !options.schema
        ? (0, plugin_helpers_1.getCachedDocumentNodeFromSchema)(schemaInstance)
        : options.schema;
    ...
    if (schemaInstance &&
        ...
        const errors = await profiler.run(() => {
            ...
            if (!schemaHash ||
                ...
                return Promise.resolve((0, utils_1.validateGraphQlDocuments)(schemaInstance, [
            ...
            const cacheKey = [schemaHash]
                ...
                .join(',');
            return options.cache('documents-validation', cacheKey, () => Promise.resolve((0, utils_1.validateGraphQlDocuments)(schemaInstance, [
                ...transformedDocuments.flatMap(d => d.document),
                ...fragments.flatMap(f => f.document),
            ], rules)));
