...
__webpack_require__.d(__webpack_exports__, {
    ...
    relativeDateParseForFeatureFlagMatching: ()=>core_namespaceObject.relativeDateParseForFeatureFlagMatching,
    InconclusiveMatchError: ()=>core_namespaceObject.InconclusiveMatchError,
...
class FeatureFlagsPoller {
    ...
    async getFeatureFlag(key, distinctId, groups = {}, personProperties = {}, groupProperties = {}) {
        ...
        if (void 0 !== featureFlag) {
            ...
            try {
                ...
                if (e instanceof RequiresServerEvaluation || e instanceof core_namespaceObject.InconclusiveMatchError) this.logMsgIfDebug(()=>console.debug(`${e.name} when computing flag locally: ${key}: ${e.message}`));
    ...
    async getAllFlagsAndPayloads(evaluationContext, flagKeysToExplicitlyEvaluate) {
        ...
        await Promise.all(flagsToEvaluate.map(async (flag)=>{
            try {
                ...
                if (e instanceof RequiresServerEvaluation || e instanceof core_namespaceObject.InconclusiveMatchError) this.logMsgIfDebug(()=>console.debug(`${e.name} when computing flag locally: ${flag.key}: ${e.message}`));
    ...
    async computeFlagValueLocally(flag, evaluationContext) {
        ...
        if (flag.ensure_experience_continuity) throw new core_namespaceObject.InconclusiveMatchError('Flag has experience continuity enabled');
        ...
        if (void 0 != aggregation_group_type_index) {
            ...
            if (!groupName) {
                ...
                throw new core_namespaceObject.InconclusiveMatchError('Flag has unknown group type index');
            }
        ...
        }
        {
            ...
            if (void 0 === bucketingValue) {
                ...
                throw new core_namespaceObject.InconclusiveMatchError(`Can't compute feature flag: ${flag.key} without $device_id`);
            }
    ...
    getFeatureFlagPayload(key, flagValue) {
        return (0, core_namespaceObject.resolveFeatureFlagPayload)(this.featureFlagsByKey?.[key]?.filters?.payloads, flagValue);
    }
    async evaluateFlagDependency(property, properties, evaluationContext) {
        const { evaluationCache } = evaluationContext;
        const targetFlagKey = property.key;
        if (!this.featureFlagsByKey) throw new core_namespaceObject.InconclusiveMatchError('Feature flags not available for dependency evaluation');
        if (!('dependency_chain' in property)) throw new core_namespaceObject.InconclusiveMatchError(`Flag dependency property for '${targetFlagKey}' is missing required 'dependency_chain' field`);
        const dependencyChain = property.dependency_chain;
        if (!Array.isArray(dependencyChain)) throw new core_namespaceObject.InconclusiveMatchError(`Flag dependency property for '${targetFlagKey}' has an invalid 'dependency_chain' (expected array, got ${typeof dependencyChain})`);
        if (0 === dependencyChain.length) throw new core_namespaceObject.InconclusiveMatchError(`Circular dependency detected for flag '${targetFlagKey}' (empty dependency chain)`);
        for (const depFlagKey of dependencyChain){
            if (!(depFlagKey in evaluationCache)) {
                ...
                if (depFlag) if (depFlag.active) try {
                    ...
                    throw new core_namespaceObject.InconclusiveMatchError(`Error evaluating flag dependency '${depFlagKey}' for flag '${targetFlagKey}': ${error}`);
                }
                else evaluationCache[depFlagKey] = false;
                else if (this.filteredOutFlagKeys.has(depFlagKey)) evaluationCache[depFlagKey] = false;
                else throw new core_namespaceObject.InconclusiveMatchError(`Missing flag dependency '${depFlagKey}' for flag '${targetFlagKey}'`);
            }
            const cachedResult = evaluationCache[depFlagKey];
            if (null == cachedResult) throw new core_namespaceObject.InconclusiveMatchError(`Dependency '${depFlagKey}' could not be evaluated`);
        }
    ...
    async matchFeatureFlagProperties(flag, bucketingValue, properties, evaluationContext) {
        ...
        for (const condition of flagConditions)try {
            ...
            if (e instanceof core_namespaceObject.InconclusiveMatchError) isInconclusive = true;
            else throw e;
        }
        if (void 0 !== result) return result;
        if (isInconclusive) throw new core_namespaceObject.InconclusiveMatchError("Can't determine if feature flag is enabled or not with given properties");
        return false;
    }
    async isConditionMatch(flag, bucketingValue, condition, properties, evaluationContext) {
        ...
        if (void 0 != rolloutPercentage && await (0, core_namespaceObject.getFeatureFlagHash)(flag.key, bucketingValue) > rolloutPercentage / 100.0) return 'out_of_rollout_bound';
        return 'match';
    }
    async getMatchingVariant(flag, bucketingValue) {
        return (0, core_namespaceObject.getFeatureFlagVariant)(flag.key, bucketingValue, flag.filters?.multivariate?.variants || []);
    }
    variantLookupTable(flag) {
        return (0, core_namespaceObject.getFeatureFlagVariantLookupTable)(flag.filters?.multivariate?.variants || []);
    }
    filterFlagsByEvaluationContexts(flags) {
        if (!this.evaluationContexts || 0 === this.evaluationContexts.length) return flags;
...
function matchProperty(property, propertyValues, warnFunction) {
    return (0, core_namespaceObject.matchFeatureFlagProperty)(property, propertyValues, {
        warnFunction
    });
}
function parseSemver(value) {
    return (0, core_namespaceObject.parseFeatureFlagSemver)(value, 'strict');
}
...
async function matchPropertyGroup(propertyGroup, propertyValues, cohortProperties, debugMode = false, flagDependencyEvaluator) {
    ...
    if ('values' in properties[0]) {
        for (const prop of properties)try {
            ...
            if (err instanceof core_namespaceObject.InconclusiveMatchError) {
                if (debugMode) console.debug(`Failed to compute property ${prop} locally: ${err}`);
                errorMatchingLocally = true;
            } else throw err;
        }
        if (errorMatchingLocally) throw new core_namespaceObject.InconclusiveMatchError("Can't match cohort without a given cohort property value");
    ...
    for (const prop of properties)try {
        ...
        if ('cohort' === prop.type) matches = await matchCohort(prop, propertyValues, cohortProperties, debugMode, flagDependencyEvaluator);
            ...
            if (!flagDependencyEvaluator) throw new core_namespaceObject.InconclusiveMatchError(`Flag dependency '${prop.key || 'unknown'}' cannot be evaluated without a flag dependency evaluator`);
