...
function createForm(config) {
    ...
    let { debug, destroyOnUnregister, keepDirtyOnReinitialize, initialValues, mutators, onSubmit, validate, validateOnBlur, callbackScheduler, ignoreUnregister, } = config;
    if (!onSubmit) {
    ...
    const calculateNextFormState = () => {
        ...
        if (!foundDirty) {
            ...
            for (const key of allKeys) {
                if (!safeFields[key]) {
                    const currentValue = (_a = formState.values) === null || _a === void 0 ? void 0 : _a[key];
                    const initialValue = (_b = formState.initialValues) === null || _b === void 0 ? void 0 : _b[key];
    ...
    const api = {
        ...
        initialize: (data) => {
            ...
            const savedDirtyValues = keepDirtyOnReinitialize
                ? Object.keys(safeFields).reduce((result, key) => {
                    ...
                    if (!pristine) {
                        ...
                        const hasChildFields = Object.keys(safeFields).some((otherKey) => otherKey !== key && (otherKey.startsWith(key + '[') || otherKey.startsWith(key + '.')));
                        if (!hasChildFields) {
            ...
            Object.keys(safeFields).forEach((key) => {
                const field = safeFields[key];
                const currentValue = getIn(formState.values, key);
                const initialValue = getIn(formState.initialValues || {}, key);
        ...
        registerField: (name, subscriber, subscription = {}, fieldConfig) => {
            ...
            if (fieldConfig) {
                ...
                const currentValue = getIn(state.formState.values, name);
                const currentInitialValue = getIn(state.formState.initialValues, name);
            ...
            return () => {
                ...
                let lastOne = hasFieldSubscribers &&
                    ...
                    state.fieldSubscribers[name].entries &&
